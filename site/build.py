...
"""Build the NexusPoint Systems bespoke site.
...
Usage:
    ...
    ./site/build.py serve        # build then serve on :8002 for local preview
...
def build():
    print(f"Building NexusPoint Systems → {DIST_DIR}")
    ...
    if not cname_file.exists():
        cname_file.write_text("nexuspointsystems.eduserver.au\n", encoding="utf-8")
        print("  ✓ CNAME")
...
def serve():
    """Build then serve the dist directory on :8002 for local preview."""
    import http.server
    import socketserver
    import os
    ...
    with socketserver.TCPServer(("", PORT), http.server.SimpleHTTPRequestHandler) as httpd:
        print(f"\nServing NexusPoint at http://localhost:{PORT}")
